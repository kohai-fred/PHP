...
if(!empty($_POST)){  // si le formulaire a été envoyé
    ...
    if(empty($contenu)){  // si la variable est vide, c'est qu'il n'y a pas de message d'erreur sur le formulaire
        ...
        if($resultat->rowCount() > 0){ // si la requête retourne 1 ou plusieurs lignes c'est que le pseudo est déjà dans la BDD.
            ...
            $mdp = password_hash($_POST['mdp'], PASSWORD_DEFAULT);  // si nous "hashons" le mot de passe (ici avec l'algorithme bcrypt par défaut), il faudra sur la page de connexion comparer le hash de la BDD avec celui du mdp fourni par l'internaute lors de la connexion.
            //debug($mdp);
...
<form method="post" action="" class="">
    ...
    <div>
        ...
        <div><input type="password" name="mdp" id="mdp" value="<?php echo $membre_actuel['mdp'] ?? '';  ?>"></div>
        ...
        <div>
            ...
            <div><input type="radio" name="civilite" value="f" <?php if(isset($membre_actuel['civilite']) && $membre_actuel['civilite'] == 'f') echo 'checked'; ?>>Femme</div>
        </div>


        <div><label for="ville">Ville</label></div>
        <div><input type="text" name="ville" id="ville" value="<?php echo $membre_actuel['ville'] ?? '';  ?>"></div>

        <div><label for="code_postal">Code postal</label></div>
        <div><input type="text" name="code_postal" id="code_postal" value="<?php echo $membre_actuel['code_postal'] ?? '';  ?>"></div>

        <div><label for="adresse">Adresse</label></div>
        <div><textarea name="adresse" id="adresse" cols="23" rows="2"><?php echo $membre_actuel['adresse'] ?? "";  ?></textarea></div>

        <div>
            <input type="submit" value="S'inscrire" class="btn mt-4">
        </div>
